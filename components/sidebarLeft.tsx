...
import { BsMegaphoneFill, BsHouseFill } from "react-icons/bs";
  ...
  if (session) {
    user_menu = (
      ...
      >
        ...
        <span className="ml-3 text-xl font-semibold">{session.user?.name}</span>
      </div>
    );
  } else {
    user_menu = (
  ...
  return (
    <div className="sticky top-0 h-screen w-3/12">
      <div className="mx-auto flex h-full w-11/12 flex-col">
        ...
        <div className="mt-5 w-full text-2xl font-semibold">
        ...
        <div
          className="absolute left-10 bottom-0 z-10 mb-20 w-56 origin-bottom rounded-md bg-gray-800 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
          role="menu"
          hidden={show}
